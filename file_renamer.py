...
def rename_files_in_folder(folder_path, custom_name):
    # Get a list of all files in the folder
    files = [f for f in os.listdir(folder_path) if os.path.isfile(
        os.path.join(folder_path, f))]
    ...
    for index, filename in enumerate(files, start=1):
        ...
        # Create the new file name with underscore
        new_filename = f"{custom_name}_{index}{file_extension}"
...
if __name__ == "__main__":
    ...
    if not os.path.exists(folder_path):
        ...
        custom_name = input("Enter the name you want for your files: ").strip()
        
        # Call the function to rename files
        rename_files_in_folder(folder_path, custom_name)